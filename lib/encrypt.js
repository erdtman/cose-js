...
exports.create = function (headers, payload, recipients, options) {
  return new Promise((resolve, reject) => {
    ...
    if (Array.isArray(recipients)) {
      ...
      const nodeAlg = COSEAlgToNodeAlg[TagToAlg[alg]];
      const cipher = crypto.createCipheriv(nodeAlg, recipients.key, iv);
...
exports.read = function (data, key, options) {
  ...
  .then((obj) => {
    ...
    const algtag = (p !== EMPTY_BUFFER) ? p.get(common.HeaderParameters.alg) : (u !== EMPTY_BUFFER) ? u.get(common.HeaderParameters.alg) : undefined;
    if (!TagToAlg[algtag]) {
      throw new Error('Unknown or unsupported algorithm ' + algtag);
    }
    const alg = COSEAlgToNodeAlg[TagToAlg[algtag]];
