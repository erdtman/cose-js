...
const nodeAesCcm = require('node-aes-ccm');
const common = require('./common');

const Tagged = cbor.Tagged;
const encrypt = nodeAesCcm.encrypt;
const decrypt = nodeAesCcm.decrypt;
...
const isCCMAlg = {
  ...
  // 11: true, // AES-CCM-16-64-256 not supported
  12: true, // AES-CCM-64-64-128
  // 13: true, // AES-CCM-64-64-256 not supported
  30: true, // AES-CCM-16-128-128
  // 31: true, // AES-CCM-16-128-256 not supported
...
const authTagLength = {
  ...
  // 11: 8, // AES-CCM-16-64-256 not supported
  12: 8, // AES-CCM-64-64-128
  // 13: 8, // AES-CCM-64-64-256 not supported
  30: 16, // AES-CCM-16-128-128
  // 31: 16, // AES-CCM-16-128-256 not supported
  32: 16 // AES-CCM-64-128-128
  // 33: 16  // AES-CCM-64-128-256 not supported
};
...
const ivLenght = {
  ...
  // 11: 13, // AES-CCM-16-64-256 not supported
  12: 7,  // AES-CCM-64-64-128
  // 13: 7,  // AES-CCM-64-64-256 not supported
  30: 13, // AES-CCM-16-128-128
  // 31: 13, // AES-CCM-16-128-256 not supported
  32: 7  // AES-CCM-64-128-128
  // 33: 7   // AES-CCM-64-128-256 not supported
};
...
function nodeEncrypt (payload, key, alg, iv, aad) {
  const nodeAlg = COSEAlgToNodeAlg[TagToAlg[alg]];
  const cipher = crypto.createCipheriv(nodeAlg, key, iv);
  cipher.setAAD(aad);
...
function ccmEncrypt (payload, key, alg, iv, aad) {
  const tagLength = authTagLength[alg];
  const ct = encrypt(key, iv, payload, aad, tagLength);
  return Buffer.concat([ct.ciphertext, ct.auth_tag]);
}
...
exports.create = function (headers, payload, recipients, options) {
  return new Promise((resolve, reject) => {
    ...
    if (Array.isArray(recipients)) {
      ...
      let ciphertext;
      if (isNodeAlg[alg]) {
        ciphertext = nodeEncrypt(payload, recipients[0].key, alg, iv, aad);
      } else if (isCCMAlg[alg]) {
        ciphertext = ccmEncrypt(payload, recipients[0].key, alg, iv, aad);
      } else {
      ...
      if (options.excludetag) {
        resolve(cbor.encode([p, u, ciphertext, recipient]));
      } else {
      ...
      let ciphertext;
      if (isNodeAlg[alg]) {
        ciphertext = nodeEncrypt(payload, recipients.key, alg, iv, aad);
      } else if (isCCMAlg[alg]) {
        ciphertext = ccmEncrypt(payload, recipients.key, alg, iv, aad);
      } else {
        throw new Error('No implementation for algorithm, ' + alg);
      }
      ...
      if (options.excludetag) {
        resolve(cbor.encode([p, u, ciphertext]));
      } else {
...
function nodeDecrypt (ciphertext, key, alg, iv, tag, aad) {
  const nodeAlg = COSEAlgToNodeAlg[TagToAlg[alg]];
  const decipher = crypto.createDecipheriv(nodeAlg, key, iv);
  decipher.setAuthTag(tag);
  decipher.setAAD(aad);
  return Buffer.concat([decipher.update(ciphertext), decipher.final()]);
}

function ccmDecrypt (ciphertext, key, alg, iv, tag, aad) {
  const pt = decrypt(key, iv, ciphertext, aad, tag);
  if (!pt.auth_ok) {
    throw new Error('Auth tag missmatch!');
  }
...
exports.read = function (data, key, options) {
  ...
  .then((obj) => {
    ...
    if (isNodeAlg[alg]) {
      ...
      return ccmDecrypt(ciphertext, key, alg, iv, tag, aad);
    } else {
      throw new Error('No implementation for algorithm, ' + alg);
    }
  });
